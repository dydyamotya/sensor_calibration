...
class CalibrationWidget(QtWidgets.QWidget):
    def __init__(self, parent, settings):
        ...
        left_layout.addWidget(self.calibration_settings)
        left_layout.addWidget(QtWidgets.QLabel("R0 Voltage"))
        left_layout.addWidget(self.r0_voltage)
        left_layout.addWidget(self.cal_buttons)
    ...
    def stop_ms(self):
        ...
        if self.thread:
            self.thread.join()
            self.ms.full_request((0, ) * self.sensor_number)
        if self.ms:
            self.ms.close()
        self.ms = None
    ...
    def get_r0(self):
        ...
        try:
            us, rs = self.ms.full_request(
                (r0_voltage, ) * self.sensor_number)
            time.sleep(2)
            ...
            self.per_sensor.set_r0s(np.mean(averaging_massive, axis=1))
        finally:
            self.ms.close()
            self.ms = None
    ...
    def loop_ms(self):
        ...
        for idx, voltage_dot in enumerate(voltage_row):
            ...
            us, rs = self.ms.full_request(
                (voltage_dot, ) * self.sensor_number)
            ...
            self.resistances[:, idx] = np.mean(averaging_massive, axis=1)
    ...
    def get_data(self):
        ...
        temperatures = self.per_sensor.process_resistances(self.resistances[:, :self.last_idx])
...
class SaveButtons(QtWidgets.QWidget):
    def __init__(self, parent):
        ...
        self.save_calibration_button = QtWidgets.QPushButton("Save calibration", self)
        self.save_parameters_button = QtWidgets.QPushButton("Save parameters", self)
    ...
    def save_calibration(self):
        filename, filters = QtWidgets.QFileDialog.getSaveFileName(self, "Save Calibration", "./tests", "Calibration File (*.cal)")
        if filename:
            ...
            with open(filename, "w") as fd:
                ...
                np.savetxt(fd, new_array, fmt='%.6f', delimiter="\t", newline="\t\n")

    def save_parameters(self):
        filename, filters = QtWidgets.QFileDialog.getSaveFileName(self, "Save Parameters", "./tests", "Parameters File (*.par)")
        if filename:
            r0s, rns, alphas, t0, Vmax = self.parent_py.get_params()
            with open(filename, "w") as fd:
                fd.write("[R0]\n")
                for idx, r0 in enumerate(r0s):
                    to_file = "R0_{:d}={:f}\n".format(idx, r0*100).replace(".", ",")
                    fd.write(to_file)
                fd.write("[Rc]\n")
                for idx, rn in enumerate(rns):
                    to_file = "Rc_{:d}={:f}\n".format(idx, rn*100).replace(".", ",")
                    fd.write(to_file)
                fd.write("[a]\n")
                for idx, alpha in enumerate(alphas):
                    to_file = "a0_{:d}={:f}\n".format(idx, alpha).replace(".", ",")
                    fd.write(to_file)
                fd.write("[T0]\n")
                to_file = "T0={:f}\n".format(t0).replace(".", ",")
                fd.write(to_file)
